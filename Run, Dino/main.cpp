...
int main()
{
	...
	bool gameOver = false, used = false, paused = false;
	srand(std::time(0));
	sf::Sprite pwrUp1;
	sf::Sprite pwrUp2;
	sf::Texture pwr1;
	...
	while (window.isOpen())
	{
		...
		if (!gameOver) {
			if (!paused) {
				...
				if (!mainHero.isJump) {//åñëè ÍÅ Â ÏÐÛÆÊÅ
					if (sf::Keyboard::isKeyPressed(sf::Keyboard::Space)) {//íàæàòèå ïðîáåëà
						jumpSound.play();
						mainHero.isJump = true; //òóò êàðî÷å áóë êîòîðûé îòâå÷àåò çà ïðûæîê ïîäðóáàåòñÿ
					}
				}
				else {
					mainHero.jump(time); //â ýòîé ôóíêöè
				}
